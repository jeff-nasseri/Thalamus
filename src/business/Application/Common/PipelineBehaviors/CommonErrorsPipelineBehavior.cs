    ...
    where TResponse : struct, IResponse
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (errorList.Any())
        {
            CommonErrorCode error = CommonErrorCode.ValidationFailed;
        ...
        {
            Enum code = response.Error!.Value.ErrorEnum;
            response.Error.Value.Values!.Add(ApplicationKeys.ERROR_SERVER_MAIN_MESSAGE, ErrorMessageUtils.GetErrorMessage(code));
        }
