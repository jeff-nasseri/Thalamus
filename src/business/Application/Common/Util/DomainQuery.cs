...
public static class DomainQuery
{
    ...
    /// <returns>A predicate function that returns true for entities within the date range, or always true if dates are invalid.</returns>
        ...
        where T : BaseEntity
    {
        ...
        bool Func(T t)
        {
            bool result = t.CreatedDateTime >= from && t.CreatedDateTime <= to;
            return result;
        }
    }
}