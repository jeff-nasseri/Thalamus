...
public class MemoryKeywordValueObject : ValueObject, IValueObjectParser<MemoryKeywordValueObject, string>
{
    ...
    {
        if (string.IsNullOrWhiteSpace(sentence)) return Enumerable.Empty<MemoryKeywordValueObject>();

        var keywords =
            sentence.Split(separator, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
